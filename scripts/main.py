...
self_folder = os.path.join(extensions_path,"extensions/sd_t2iapi")
...
def on_ui_tabs():
    with gr.Blocks(analytics_enabled=False) as ui_component:
        with gr.Column():
            pass
        return [(ui_component, "t2iapi", "t2iapi")]
    
def on_app_started(_: gr.Blocks, app: FastAPI) -> None:
    @app.get("/t2iapi/easy_ui.html",response_class=HTMLResponse)
    def welcome():
        return t2iapi_html

    @app.post("/t2iapi/api/generate")
    def t2iapi(data:dict,request: Request):
    ...
    @app.get("/t2iapi/api/easyGetOptions")
